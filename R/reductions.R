...
#' @param solution A solution to the problem emitted by 'reduce()'
#' @describeIn Retrieves a solution to the owned problem
...
#' @describeIn Canonicalization recursively canonicalizes the objective and every constraint
...
#DK CHECK MAKE SURE IT'S ACTUALLY CORRECT
#' @param solution A \linkS4class{Solution} to a problem generated by the inverse data
#' @param inverse_data Data encoded in \linkS4class{InverseData} for the original problem
#' @describeIn Returns a solution to the original problem given the inverse_data
...
#' @describeIn Recursively canonicalize an Expression
...
#' @describeIn Canonicalize an expression with respect to canonicalized arguments
...
#' @describeIn A problem is accepted if the sequence of reductions is valid. In particular,
#' the i-th reduction must accept the output of the i-1th reduction, with the first reduction 
#' (objectr at eductions[1])
...
#DK
#' @param solution A \linkS4class{Solution} (in either the Solution or list class) to a problem that generated the inverse data
#' @param inverse_data The inverse data in list form yielded by each of the reductions
#' @describein Returns a solution to the original problem given the inverse_data
...
#' @param problem The problem to check
#' @describeIn Returns true to say the problem is accepted
setMethod("accepts", signature(object = "CvxAttr2Constr", problem = "Problem"), function(object, problem) { TRUE })


#' @param problem The problem object that is iterated to convert variables to constraints
#' @describeIn CvxAttr2Constr Performs the reduction to expand convex variable attributes into constraints
...
#' @param solution A \linkS4class{Solution} to a problem that generated the inverse data
#' @param inverse_data A list containing variable data
...
#' @param problem The problem whose parameters should be evaluated
#' @describeIn Replaces symbolic parameters with their constsant values
...
#' @param solution A \linkS4class{Solution} to a problem that generated the inverse data
#' @param inverse_data List containing data from previous reduction
#' @describeIn Returns the solution given to it
...
#' @param problem The problem whose objective is to be flipped
#' @describeIn Returns the problem but with a flipped objective
...
#' @param inverse_data A list that is only returned due to an invocation to apply
#' @describeIn Maps the solution of the flipped problem to that of the original
...
#' @param problem The problem to stuff; the arguments of every constarint must be affine
#' @describeIn Returns the stuffed problem and the data for solution retrieval later
...
#' @solution Solution of the previous reduction
#' @inverse_data  Originial data in the form of \linkS4class{Inversedata} needed to conver the solution 
#' @describeIn Returns the solution to the original problem given the inverse data
